...
class Msg extends Model{
	...
	public function getMySendMsgs( $user_id ){
		...
		$sql = 'SELECT m.receive_user_id,u.name receive_user_name,m.msg_id,m.status,m.view_time,
					   mt.msg_title,mt.msg_content,mt.type,mt.create_time,s.src_id,s.access_url
	...
	public function getMyReceiveMsgs( $user_id ){
		...
		$sql = 'SELECT DISTINCT(m.msg_id),m.send_user_id,u.name send_user_name,m.msg_id,m.status,m.view_time,
					   mt.msg_title,mt.msg_content,mt.type,mt.create_time,s.src_id,s.access_url
				FROM msg AS m LEFT JOIN msg_text AS mt ON m.msg_id = mt.msg_id
	...
	public function changeSingleMsgStatus($send_user_id,$receive_user_id,$msg_id,$status){
		if( $status == 1){
			$sql = 'UPDATE msg
				SET status = :status,view_time = :view_time
				WHERE send_user_id = :send_user_id && receive_user_id = :receive_user_id && msg_id = :msg_id';
			$data = ['send_user_id'=>$send_user_id,'receive_user_id'=>$receive_user_id,'msg_id'=>$msg_id,'status' => $status,'view_time' => date('Y-m-d H:i:s',time())];
		}else{
			$sql = 'UPDATE msg
				SET status = :status
				WHERE send_user_id = :send_user_id && receive_user_id = :receive_user_id && msg_id = :msg_id';
			$data = ['send_user_id'=>$send_user_id,'receive_user_id'=>$receive_user_id,'msg_id'=>$msg_id,'status' => $status];
		}
